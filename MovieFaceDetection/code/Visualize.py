...
def show_detections_without_ground_truth(detections, scores, file_names, params: Parameters):
    ...
    if detections is None:
        print('No detections.')
        return
    test_images_path = os.path.join(params.dir_validation, 'validare', '*.jpg')
    ...
    for i, test_file in enumerate(test_files):
        ...
        for idx, detection in enumerate(current_detections):
            cv.rectangle(image, (detection[0], detection[1]), (detection[2], detection[3]), (255, 0, 255), thickness=3)
            cv.putText(image, 'score:' + str(current_scores[idx])[:4], (detection[0], detection[1]),
                       cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
        cv.imwrite(os.path.join(params.dir_save_files, 'visuals', "detections_" + short_file_name), image)
