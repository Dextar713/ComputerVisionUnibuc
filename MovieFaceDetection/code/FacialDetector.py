...
from MovieFaceDetection.code.image_processing import pyramid
...
class FacialDetector:
    ...
    def train_classifier(self, training_examples, train_labels, classify_flag=False, with_color=False):
        color = ''
        if with_color:
            color = 'color'
        svm_file_name = os.path.join(self.params.dir_save_files, 'models', 'best_model_%d_%d_%d' %
                                     (self.params.dim_hog_cell, self.params.dim_face[0],
                                      self.params.dim_face[1]))
        if classify_flag:
            svm_file_name = svm_file_name + '_class'
        if with_color:
            svm_file_name = svm_file_name + '_color'
        if os.path.exists(svm_file_name):
            self.best_model = pickle.load(open(svm_file_name, 'rb'))
            return

        # train_end = int(0.82 * len(training_examples))
        # X_train, X_val = training_examples[:train_end], training_examples[train_end:]
        # y_train, y_val = train_labels[:train_end], train_labels[train_end:]
        X_train = training_examples
    ...
    def run(self):
        ...
        """
        if not is_test:
            test_images_path = os.path.join(self.params.dir_validation, 'validare', '*.jpg')
        else:
            test_images_path = os.path.join(self.params.dir_test_examples, 'testare', '*.jpg')
        ...
        for i in range(num_test_images):
            ...
            image_scores = []
            image_detections = []
            face_size = self.params.dim_face
            for scaled_img, (scale_w, scale_h), (_, _) in pyramid(img, self.params.scale_factor_w, self.params.scale_factor_h, face_size):
                hog_descriptors = hog(scaled_img, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell),
                                  cells_per_block=(2, 2), feature_vector=False)
                ...
                num_cell_in_template_w = self.params.dim_face[0] // self.params.dim_hog_cell - 1
                num_cell_in_template_h = self.params.dim_face[1] // self.params.dim_hog_cell - 1
                ...
                for y in range(0, num_rows - num_cell_in_template_h):
                    for x in range(0, num_cols - num_cell_in_template_w):
                        ...
                        #          ]
                        descr = hog_descriptors[y:y + num_cell_in_template_h, x:x + num_cell_in_template_w].flatten()
                        # descr = hog(window, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell),
                        #                        cells_per_block=(2, 2), feature_vector=True)
                        score = np.dot(descr, w)[0] + bias
                        if score > self.params.threshold:
                            x_min_scaled = int(x * self.params.dim_hog_cell)
                            y_min_scaled = int(y * self.params.dim_hog_cell)
                            x_max_scaled = int(x * self.params.dim_hog_cell + self.params.dim_face[0])
                            y_max_scaled = int(y * self.params.dim_hog_cell + self.params.dim_face[1])
                            scale_inv_w = 1.0 / scale_w
                            scale_inv_h = 1.0 / scale_h
                            x_min = int(x_min_scaled * scale_inv_w)
                            y_min = int(y_min_scaled * scale_inv_h)
                            x_max = int(x_max_scaled * scale_inv_w)
                            y_max = int(y_max_scaled * scale_inv_h)
